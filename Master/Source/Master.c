...
static void vProcessEvCore(tsEvent *pEv, teEvent eEvent, uint32 u32evarg) {
	...
	case E_STATE_PER:
		...
		if (eEvent == E_EVENT_TICK_A && !sAppData.bTxBusy) {
			...
			// １－２バイト目は制御情報
			tsTx.auData[0] = sAppData.u8Power;
	...
	case E_STATE_PER_FINISH:
		if (ToCoNet_Event_u32TickFrNewState(pEv) > 250) {
			uint32 u32permil, u32permilAppAck;
...
static void vInitHardware(int f_warm_start) {
	...
	sTimerApp.u8PreScale = 5;
	vTimerConfig(&sTimerApp);
	vTimerStart(&sTimerApp);

#ifdef USE_SERIAL1
	vfPrintf(&sSerStream, "\r\n*** ToCoNet PER MASTER %d.%02d-%d ***", VERSION_MAIN, VERSION_SUB, VERSION_VAR);
...
static void vHandleSerialInput(int16 i16CharExt) {
	...
	while (!SERIAL_bRxQueueEmpty(sSerPort.u8SerialPort)
		...
		case 'P':
			_C {
				const uint16 au16count_table[] = { PER_DEF_PAYLOAD, 6, 8, 16, 32,
						64, 96, 108, 0 };
				static uint8 u8idx = 0;
		...
		case 'O':
			_C {
				const uint16 au16pow_table[] = { 3, 0, 1, 2, 3, 0 };
				static uint8 u8idx = 0;

				u8idx++;
				if (au16pow_table[u8idx] == 0) {
					u8idx = 1;
				}
		...
		case 'T':
			_C {
				const uint16 au16count_table[] = { TIMER_TICK_MS, 5, 10, 20, 33, 50,
						67, 100, 0 };
				static uint8 u8idx = 0;
...
void vUpdateLcdOpt() {
	vfPrintf(&sLcdStream, "\f< %s PER %d.%02d-%d >\n\r",
			...
			sAppData.u8Lang == MSG_LANG_JAPANESE ? (char*)au8Tocos : "TOCOS",
